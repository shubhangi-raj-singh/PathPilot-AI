  ...
  return (
    <div className="tool-page">
      ...
      <div className="tool-card">
        ...
        <p>Discover career paths based on your skills, interests and strengths.</p>

        <div className="tool-form">
          <input placeholder="Your Skills" value={skills} onChange={(e) => setSkills(e.target.value)} />
          <input placeholder="Your Interests" value={interests} onChange={(e) => setInterests(e.target.value)} />
          ...
          <button className="generate-btn" onClick={handleGenerate} disabled={loading}>
            {loading ? "Generating Report..." : "Generate Analysis"}
          </button>

          {output && (
            <>
              <div className="roadmap-output" style={{ marginTop: "30px", width: "100%", textAlign: "left" }}>
                <ReactMarkdown>{output}</ReactMarkdown>
              </div>
              <button className="pdf-btn" onClick={downloadPDF} style={{ marginTop: "20px" }}>📄 Download PDF</button>
            </>
          )}
          <div className="tool-note">✦ AI analyzes your profile dynamically based on inputs.</div>
        </div>
      </div>
    </div>
  );
